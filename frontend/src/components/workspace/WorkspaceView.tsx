...
export const WorkspaceView = ({ sessionId, initialLanguage: incomingLanguage, initialPrompt: incomingPrompt = null, initialModel: incomingModel = null, initialPlatform: incomingPlatform, initialCategory: incomingCategory, onExit, activeModal: propActiveModal, onSetActiveModal }: WorkspaceViewProps) => {
    ...
    // Restore platform/language/category from localStorage if URL params missing
    ...
    const loadSettings = async () => {
        try {
            ...
            if (settings) {
                ...
                // Create .project.json if not present
                setFiles(prev => {
                    if (prev['.project.json']) return prev;
                    const meta = {
                        platform: settings.platform || platform,
                        category: settings.category || category,
                        language: settings.language || language,
                    };
                    const content = JSON.stringify(meta, null, 4);
                    fileApi.saveFiles(sessionId, { '.project.json': content }).catch(() => {});
                    return { ...prev, '.project.json': content };
                });
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            localStorage.setItem('velix_session_prefs', JSON.stringify({ platform, language, category }));
            if (sessionId) {
                localStorage.setItem(`velix_session_${sessionId}`, JSON.stringify({ platform, language, category }));
            }
        }
    }, [platform, language, category, sessionId]);
    ...
    // Load metadata from .project.json after files are loaded (read-only, runs once)
    useEffect(() => {
        if (!sessionId || metaLoaded) return;
        const metaContent = files['.project.json'];
    ...
    }, [sessionId, files, metaLoaded]);

    // Write-back: persist user changes to .project.json and database AFTER initial load
    useEffect(() => {
        if (!sessionId || !metaLoaded) return;
        const meta = { platform, category, language };
        const content = JSON.stringify(meta, null, 4);
        setFiles(prev => {
            if (prev['.project.json'] === content) return prev;
            return { ...prev, '.project.json': content };
        });
        fileApi.saveFiles(sessionId, { '.project.json': content }).catch(() => {});
        aiApi.getProjectSettings(sessionId).then(({ settings = {} }) => {
            aiApi.updateProjectSettings(sessionId, { ...settings, platform, category, language }).catch(() => {});
        }).catch(() => {});
    }, [sessionId, metaLoaded, platform, category, language]);
                                ...
                                <SettingsContent title="Overview" subtitle="Session statistics and information">
                                    ...
                                    <div className="neu-inset rounded-xl p-5">
                                        ...
                                        <div className="space-y-3 text-xs">
                                            ...
                                            <InfoRow label="Platform" value={platform === 'minecraft' ? 'Minecraft' : platform === 'discord' ? 'Discord' : platform === 'hytale' ? 'Hytale' : platform === 'chrome' ? 'Chrome Extension' : platform} />
                                            <InfoRow label="Category" value={category === 'plugins' ? 'Plugin' : category === 'mods' ? 'Mod' : category === 'bots' ? 'Bot' : category === 'extension' ? 'Extension' : category === 'configuration' ? 'Configuration' : category === 'scripting' ? 'Scripting' : category === 'datapacks' ? 'Data Pack' : category} />
                                            <InfoRow label="Language" value={
                                                 ...
                                                 language === 'ruby' ? 'Ruby' :
                                                 language === 'forge-java' ? 'Forge (Java)' :
                                                 language === 'fabric-java' ? 'Fabric (Java)' :
                                                 language?.startsWith('config-') ? language.replace('config-', '').replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) + ' Config' :
                                                 language?.startsWith('scripting-') ? language.replace('scripting-', '').replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) :
                                                 language?.startsWith('datapack-') ? language.replace('datapack-', '').replace(/-/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) :
                                                 language
                                             } />
