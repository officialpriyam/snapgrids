...
export default function AdminPanel({ initialEditor }: { initialEditor?: InitialEditor }) {
  const [authLoading, setAuthLoading] = useState(true)
  const [authenticated, setAuthenticated] = useState(false)
  const [settings, setSettings] = useState<AdminSettings | null>(null)
  const [activeEditor, setActiveEditor] = useState<ActiveEditor>(null)
  const [appliedInitialEditor, setAppliedInitialEditor] = useState("")
  ...
  useEffect(() => {
    if (!settings || !initialEditor) {
      return
    }

    const key = JSON.stringify(initialEditor)
    if (appliedInitialEditor === key) {
      return
    }
    ...
    if (initialEditor.type === "game") {
      const index = settings.site.games.findIndex((game) => game.slug === initialEditor.slug || game.id === initialEditor.slug)
      if (index >= 0) {
    ...
    if (initialEditor.type === "service") {
      const index = settings.site.services.findIndex((service) => service.slug === initialEditor.slug || service.id === initialEditor.slug)
      if (index >= 0) {
    ...
    setActiveEditor({ type: "legal", kind: initialEditor.kind })
    setAppliedInitialEditor(key)
  }, [appliedInitialEditor, initialEditor, settings])
  ...
  function updateNavigation(patch: Partial<NavigationConfig>) {
    updateSite((site) => ({
      ...site,
      navigation: {
        ...site.navigation,
  ...
  function updateNavigationItem(index: number, patch: Partial<NavigationItem>) {
    updateSite((site) => ({
      ...site,
      navigation: {
        ...site.navigation,
        mainNavigation: site.navigation.mainNavigation.map((item, itemIndex) =>
  ...
  function addNavigationItem() {
    updateSite((site) => ({
      ...
      navigation: {
        ...site.navigation,
        mainNavigation: [
          ...site.navigation.mainNavigation,
          {
            name: "New Page",
            href: "/",
            icon: "Globe",
  ...
  function removeNavigationItem(index: number) {
    updateSite((site) => ({
      ...
      navigation: {
        ...site.navigation,
        mainNavigation: site.navigation.mainNavigation.filter((_, itemIndex) => itemIndex !== index),
      },
    }))
  }

  function updateDropdownItem(navIndex: number, itemIndex: number, patch: Partial<DropdownItem>) {
    updateSite((site) => ({
      ...site,
      navigation: {
        ...site.navigation,
        mainNavigation: site.navigation.mainNavigation.map((item, index) => {
          if (index !== navIndex) {
            return item
          }
          ...
          return {
            ...item,
            dropdownItems: (item.dropdownItems ?? []).map((dropdownItem, dropdownIndex) =>
              dropdownIndex === itemIndex ? { ...dropdownItem, ...patch } : dropdownItem
            ),
          }
        }),
      },
  ...
  function addDropdownItem(navIndex: number) {
    updateSite((site) => ({
      ...site,
      navigation: {
        ...site.navigation,
        mainNavigation: site.navigation.mainNavigation.map((item, index) =>
          index === navIndex
            ? {
                ...item,
                hasDropdown: true,
                dropdownType: item.dropdownType ?? "custom",
                dropdownItems: [...(item.dropdownItems ?? []), { name: "New Link", href: "/", description: "" }],
              }
            : item
        ),
      },
  ...
  function removeDropdownItem(navIndex: number, itemIndex: number) {
    updateSite((site) => ({
      ...site,
      navigation: {
        ...site.navigation,
        mainNavigation: site.navigation.mainNavigation.map((item, index) =>
          index === navIndex
            ? { ...item, dropdownItems: (item.dropdownItems ?? []).filter((_, dropdownIndex) => dropdownIndex !== itemIndex) }
            : item
        ),
      },
    }))
  ...
  function updateSocialLink(index: number, patch: Partial<SocialLink>) {
    updateSite((site) => ({
      ...
      navigation: {
        ...site.navigation,
        socialLinks: site.navigation.socialLinks.map((link, itemIndex) => (itemIndex === index ? { ...link, ...patch } : link)),
      },
    }))
  }
  ...
  function addSocialLink() {
    updateSite((site) => ({
      ...
      navigation: {
        ...site.navigation,
        socialLinks: [...site.navigation.socialLinks, { name: "Discord", href: "#", icon: "discord" }],
      },
    }))
  }
  ...
  function removeSocialLink(index: number) {
    updateSite((site) => ({
      ...
      navigation: {
        ...site.navigation,
        socialLinks: site.navigation.socialLinks.filter((_, itemIndex) => itemIndex !== index),
      },
    }))
  }
    ...
    if (activeEditor.type === "navbar") {
      return (
        <NavbarEditor
          ...
          onItemChange={updateNavigationItem}
          onAddItem={addNavigationItem}
          onRemoveItem={removeNavigationItem}
          onDropdownItemChange={updateDropdownItem}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Billing URL">
                <input
                  ...
                  placeholder="Paymenter billing URL"
                  className="admin-input"
                />
              </Field>
              <Field label={`API Key ${settings.paymenter.hasApiKey ? "(stored)" : ""}`}>
                <input
                  ...
                  type="password"
                  placeholder={settings.paymenter.hasApiKey ? "Leave blank to keep current key" : "Enter API key"}
                  className="admin-input"
                />
              </Field>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {settings.site.services.map((service, index) => (
              <div key={`${service.slug}-${index}`} className="rounded-md border border-secondary p-4">
                ...
                <div className="mt-4 flex gap-2">
                  <Link
                    href={`/admin/hosting/${service.slug}/edit`}
                    className="flex flex-1 items-center justify-center gap-2 rounded-md border border-secondary px-3 py-2 text-sm"
                  >
                    <Pencil className="h-4 w-4" />
                    Edit
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {[
              ...
              <div key={item.kind} className="rounded-md border border-secondary p-4">
                ...
                <Link
                  href={`/admin/legal/${item.kind === "termsOfService" ? "terms-of-service" : "privacy-policy"}/edit`}
...
function HomepageGameBoxesEditor({
  settings,
  updateSite,
  updateGame,
  gameEditHref,
  ...
  updateGame: (index: number, patch: Partial<CmsGamePage>) => void
  gameEditHref: (game: CmsGamePage) => string
}) {
  const featuredCount = settings.site.games.filter((game) => game.featured && game.visible).length
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-md border border-secondary bg-gray-50 p-3 text-sm text-gray-600 dark:bg-gray-900/40 dark:text-gray-300">
        {featuredCount > 0
          ? `${featuredCount} visible game boxes are selected for the homepage.`
          : "No featured game boxes are selected, so the homepage will use the first visible games."}
      </div>

      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {settings.site.games.map((game, index) => (
          <div key={`${game.slug}-${index}`} className="overflow-hidden rounded-md border border-secondary bg-white dark:bg-gray-950/40">
            <div className="relative h-28 bg-gray-100 dark:bg-gray-900">
              <img src={game.banner || "/meta/Banner.png"} alt="" className="h-full w-full object-cover" />
              <div className="absolute inset-0 bg-black/35" />
              <div className="absolute left-3 top-3 h-12 w-12 overflow-hidden rounded-md border border-white/20 bg-black/50">
                <img src={game.icon || "/meta/Logo.png"} alt="" className="h-full w-full object-cover" />
              </div>
              {game.comingSoon && (
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <p className="shrink-0 text-sm font-semibold text-emerald-500">{game.startingAt}</p>
              </div>
              <div className="mt-3 flex flex-wrap gap-3">
                <ToggleField label="Show box" checked={game.featured} onChange={(value) => updateGame(index, { featured: value })} />
                <ToggleField label="Visible" checked={game.visible} onChange={(value) => updateGame(index, { visible: value })} />
              </div>
              <Link
                href={gameEditHref(game)}
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <div className="flex items-center gap-4 pt-6">
        ...
        <ToggleField label="Coming soon" checked={game.comingSoon} onChange={(value) => onChange(index, { comingSoon: value })} />
      </div>
      <Field label="Coming soon message">
        <input
          value={game.comingSoonMessage}
          onChange={(event) => onChange(index, { comingSoonMessage: event.target.value })}
          className="admin-input"
        />
      </Field>
...
function ServiceEditor({
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <Field label="Coming soon message">
        <input
          value={service.comingSoonMessage}
          onChange={(event) => onChange(index, { comingSoonMessage: event.target.value })}
          className="admin-input"
        />
      </Field>
...
function NavbarEditor({
  ...
  onChange: (patch: Partial<NavigationConfig>) => void
  onItemChange: (index: number, patch: Partial<NavigationItem>) => void
  onAddItem: () => void
  onRemoveItem: (index: number) => void
  onDropdownItemChange: (navIndex: number, itemIndex: number, patch: Partial<DropdownItem>) => void
  onAddDropdownItem: (navIndex: number) => void
  onRemoveDropdownItem: (navIndex: number, itemIndex: number) => void
  onSocialLinkChange: (index: number, patch: Partial<SocialLink>) => void
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-md border border-secondary p-4">
        <h3 className="mb-4 font-semibold">Client Button</h3>
        <div className="grid gap-3 md:grid-cols-3">
          <Field label="Button label">
            <input
              value={navigation.clientSpace.name}
              onChange={(event) => onChange({ clientSpace: { ...navigation.clientSpace, name: event.target.value } })}
              className="admin-input"
            />
          </Field>
          <Field label="Button URL">
            <input
              value={navigation.clientSpace.href}
              onChange={(event) => onChange({ clientSpace: { ...navigation.clientSpace, href: event.target.value } })}
              className="admin-input"
            />
          </Field>
          <Field label="Icon name">
            <input
              value={navigation.clientSpace.icon}
              onChange={(event) => onChange({ clientSpace: { ...navigation.clientSpace, icon: event.target.value } })}
              className="admin-input"
            />
          </Field>
        </div>
      ...
      <div className="rounded-md border border-secondary p-4">
        <h3 className="mb-4 font-semibold">Top Banner</h3>
        <div className="grid gap-3 md:grid-cols-3">
          <div className="pt-7">
            <ToggleField
              label="Show banner"
              checked={navigation.banner.show}
              onChange={(value) => onChange({ banner: { ...navigation.banner, show: value } })}
            />
          </div>
          <Field label="Banner text">
            <input
              value={navigation.banner.text}
              onChange={(event) => onChange({ banner: { ...navigation.banner, text: event.target.value } })}
              className="admin-input"
            />
          </Field>
          <Field label="Coupon / pill text">
            <input
      ...
      <div className="rounded-md border border-secondary p-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h3 className="font-semibold">Navbar Items</h3>
          <button type="button" onClick={onAddItem} className="flex items-center gap-2 rounded-md border border-secondary px-3 py-2 text-sm">
            <Plus className="h-4 w-4" />
            Add Item
          </button>
        </div>
        <div className="space-y-4">
          {navigation.mainNavigation.map((item, index) => (
            <div key={`${item.name}-${index}`} className="rounded-md border border-secondary p-3">
              <div className="mb-3 flex justify-end">
                <button type="button" onClick={() => onRemoveItem(index)} className="text-red-500" aria-label={`Remove ${item.name}`}>
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
              <div className="grid gap-3 md:grid-cols-4">
                <Field label="Label">
                  <input value={item.name} onChange={(event) => onItemChange(index, { name: event.target.value })} className="admin-input" />
                </Field>
                <Field label="URL">
                  <input value={item.href} onChange={(event) => onItemChange(index, { href: event.target.value })} className="admin-input" />
                </Field>
                <Field label="Icon name">
                  <input value={item.icon ?? ""} onChange={(event) => onItemChange(index, { icon: event.target.value })} className="admin-input" />
                </Field>
                <Field label="Dropdown type">
                  <select
                    value={item.dropdownType ?? "custom"}
                    onChange={(event) => onItemChange(index, { dropdownType: event.target.value as NavigationItem["dropdownType"] })}
                    className="admin-input"
                  >
              ...
              <div className="mt-2">
                <ToggleField label="Has dropdown" checked={Boolean(item.hasDropdown)} onChange={(value) => onItemChange(index, { hasDropdown: value })} />
              </div>
              {item.hasDropdown && item.dropdownType === "custom" && (
                <div className="mt-4 rounded-md border border-secondary p-3">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <h4 className="text-sm font-semibold">Dropdown Links</h4>
                    <button
                      type="button"
                      onClick={() => onAddDropdownItem(index)}
                      className="flex items-center gap-2 rounded-md border border-secondary px-3 py-2 text-sm"
                    >
                      <Plus className="h-4 w-4" />
                      Add Link
                    </button>
                  ...
                  <div className="space-y-2">
                    {(item.dropdownItems ?? []).map((dropdownItem, itemIndex) => (
                      <div key={`${dropdownItem.name}-${itemIndex}`} className="grid gap-2 md:grid-cols-[1fr_1fr_1fr_auto]">
                        <input
                          value={dropdownItem.name}
                          onChange={(event) => onDropdownItemChange(index, itemIndex, { name: event.target.value })}
                          className="admin-input"
                        />
                        <input
                          value={dropdownItem.href}
                          onChange={(event) => onDropdownItemChange(index, itemIndex, { href: event.target.value })}
                          className="admin-input"
                        />
                        <input
                          value={dropdownItem.description ?? ""}
                          onChange={(event) => onDropdownItemChange(index, itemIndex, { description: event.target.value })}
                          className="admin-input"
                        />
                        <button type="button" onClick={() => onRemoveDropdownItem(index, itemIndex)} className="text-red-500">
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </div>
                    ))}
      ...
      <div className="rounded-md border border-secondary p-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={onAddSocialLink} className="flex items-center gap-2 rounded-md border border-secondary px-3 py-2 text-sm">
            <Plus className="h-4 w-4" />
            Add Social
          </button>
        </div>
        <div className="space-y-3">
          {navigation.socialLinks.map((link, index) => (
            <div key={`${link.name}-${index}`} className="grid gap-2 md:grid-cols-[1fr_1fr_1fr_auto]">
              <input value={link.name} onChange={(event) => onSocialLinkChange(index, { name: event.target.value })} className="admin-input" />
              <input value={link.href} onChange={(event) => onSocialLinkChange(index, { href: event.target.value })} className="admin-input" />
              <input value={link.icon} onChange={(event) => onSocialLinkChange(index, { icon: event.target.value })} className="admin-input" />
              <button type="button" onClick={() => onRemoveSocialLink(index)} className="text-red-500">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
          ))}
