...
const getGameHref = (gameId: string) => `/games/${gameId.toLowerCase().replace(/[^a-z0-9-]/g, '')}`;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3  sm:gap-4">
            {filteredGames.map((game: any) => (
              <Link
                ...
                aria-label={`View ${game.name} server options`}
                prefetch={true}
              >
                <Image
                  src={game.banner}
                    ...
                    {filteredGames.map((game: any) => (
                      ...
                      >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
                          <h3 className="text-white text-xs font-semibold truncate">{game.name}</h3>
                        </div>
                      </Link>
                    ))}
                  </div>
