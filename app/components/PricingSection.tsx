...
function homepageKeyForPlan(link: string, titleKey: string) {
  if (titleKey === "discord.title") return "discord"
  if (link === "/games") return "gameServers"
  if (link === "/vps") return "vpsHosting"
  if (link === "/dedicated") return "dedicatedServers"
  if (link === "/webhosting") return "webHosting"
  return ""
}
      ...
      <div className="relative  z-10 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-4 items-start">
          {config.section.plans.map((plan, index) => {
            const homepageKey = homepageKeyForPlan(plan.link, plan.titleKey)
            const paymenterPlans = homepageKey ? paymenterPlansByKey[homepageKey]?.plans ?? [] : []
            const startingPrice = paymenterPlans[0]?.price ?? plan.basePrice.toString()
              ...
              <div className="p-4">
                <div className="text-center mb-6 sm:mb-8">
                  <div className="mb-4 sm:mb-6">
                    ...
                    <span className="text-3xl sm:text-4xl orbitron-font font-bold text-gray-900 dark:text-white">{convertPrice(startingPrice)}</span>
                    <span className="text-sm text-gray-500 dark:text-gray-400">{paymenterPlans[0]?.period ?? "/month"}</span>
                  </div>
                </div>
