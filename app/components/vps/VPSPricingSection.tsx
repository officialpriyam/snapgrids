...
import { paymenterStockLabel, usePaymenterPlans } from "../../hooks/usePaymenterPlans"

const config = vpsConfig as VPSConfig

type VPSPricingSectionProps = {
  pageConfig?: CmsServicePage
}

export default function VPSPricingSection({ pageConfig }: VPSPricingSectionProps) {
  ...
  const paymenterPlans = usePaymenterPlans("page", pageConfig?.slug || "vps")
  ...
  const staticPlans = config.plans[selectedCPU] || config.plans[config.planTypes[0].id]
    ...
    >
      <div className="absolute inset-0">
        <div
          ...
          style={{
            backgroundImage: `url('${pageConfig?.banner || "/vps/vps-hero-2.webp"}')`,
          }}
        />
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
            <div className="flex-1">
              <div className="inline-flex items-left gap-2 card-primary px-4 py-1 rounded-tl-2xl rounded-br-2xl mb-4 border border-secondary">
                <span className="icon-text-primary text-sm">{pageConfig?.label || t('vps.badge')}</span>
              </div>
              <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-4 orbitron-font">
                {(pageConfig?.name || t('vps.title')).split(" ").slice(0, -1).join(" ")}{" "}
                <span className="icon-text-primary relative">
                  {(pageConfig?.name || t('vps.title')).split(" ").slice(-1)[0]}
                  <motion.svg
              ...
              <p className="text-md text-gray-600 max-w-3xl dark:text-gray-300">{pageConfig?.description || t('vps.description')}</p>
            </div>
            <CurrencySelector
