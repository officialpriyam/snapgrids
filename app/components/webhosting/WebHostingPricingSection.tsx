...
type WebHostingPricingSectionProps = {
  pageConfig?: CmsServicePage
}

export default function WebHostingPricingSection({ pageConfig }: WebHostingPricingSectionProps) {
  ...
  const paymenterPlans = usePaymenterPlans("page", pageConfig?.slug || "webhosting")
    ...
    >
      <div className="absolute inset-0">
        <div
          ...
          style={{
            backgroundImage: `url('${pageConfig?.banner || "/banners/webhosting.png"}')`,
          }}
        />
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-4">
            <div className="flex-1">
              <div className="inline-flex items-left gap-2 card-primary px-4 py-2 rounded-tl-2xl rounded-br-2xl mb-4 border border-secondary">
                <span className="icon-text-primary text-sm">{pageConfig?.label || t('webHostingPricing.badge')}</span>
              </div>
              <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-4 orbitron-font">
                {(pageConfig?.name || t('webHostingPricing.title')).split(" ").slice(0, -1).join(" ")}{" "}
                <span className="icon-text-primary relative">
                  {(pageConfig?.name || t('webHostingPricing.title')).split(" ").slice(-1)[0]}
                  <motion.svg
              ...
              <p className="text-md text-gray-600 max-w-3xl dark:text-gray-300">{pageConfig?.description || t('webHostingPricing.description')}</p>
            </div>
            <CurrencySelector
                ...
                <div className="p-6">
                  <div className="flex items-center gap-4 mb-6">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">{pageConfig?.label || t('webHostingPricing.webHosting')}</p>
                    </div>
                  </div>
